...
final class TargetLoad {
	...
	JsonObject toJson(boolean includeLocations, int maxProblems) {
		...
		if ("done".equals(state)) { //$NON-NLS-1$
			json.put("note", //$NON-NLS-1$
					"This is now the active target platform, and PDE recomputed the plug-in classpaths against it. Problem markers across the workspace change with it: eclipse_get_problems builds first and reports the new state, and eclipse_get_bundle_info with unresolvedOnly names the bundles that still do not resolve. BUILD FULLY BEFORE BELIEVING ANY ERROR COUNT: a target switch changes what every plug-in project compiles against without changing a single file, so an incremental build, which starts from a resource delta, can leave thousands of stale errors standing. Use eclipse_build with kind 'full', and read builderFailures in its answer, since an exception inside JavaBuilder never becomes a problem marker."); //$NON-NLS-1$
		}
		return json;
	}
}
