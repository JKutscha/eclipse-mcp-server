...
public final class ThemeDefinitionTool implements IMcpTool {
	...
	public McpToolResult call(Map<String, Object> arguments, IProgressMonitor monitor) {
		...
		Pattern id;
		String pattern = args.getString("idPattern"); //$NON-NLS-1$
		try {
			id = pattern == null ? null : Pattern.compile(pattern, Pattern.CASE_INSENSITIVE);
		...
		ThemeDefinitions.Query query = new ThemeDefinitions.Query(!"fonts".equals(kind), !"colors".equals(kind), id, //$NON-NLS-1$ //$NON-NLS-2$
				args.getString("categoryId"), args.getBoolean("onlyOverridden", false), //$NON-NLS-1$ //$NON-NLS-2$
