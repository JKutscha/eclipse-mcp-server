...
`eclipse_organize_imports` and `eclipse_format` modify the file they are given, `eclipse_write_file` creates and replaces files, `eclipse_set_target_platform` replaces what the workspace compiles against, `eclipse_build` runs builders, `eclipse_get_problems` triggers a build when auto-build is off, `eclipse_run_workbench_command` does whatever the named command's handler does, `eclipse_manage_window` opens and closes windows, `eclipse_log_status` writes into the Error Log, `eclipse_install_bundle` installs or replaces a bundle in the running framework or copies a jar into dropins, and the p2 tools `eclipse_install` and `eclipse_uninstall` change the installed software itself.
